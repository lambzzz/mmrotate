...
class ConvHead(RotatedBBoxHead):
    ...
    def __init__(self,
        ...
        # self.norm = nn.LayerNorm(self.conv_out_channels)

        # add shared convs and fcs
        self.shared_convs, self.shared_fcs, last_layer_dim = \
            self._add_convDS_branch(
    ...
    def _add_convDS_branch(self,
        ...
        if num_branch_convs > 0:
            for i in range(num_branch_convs):
                conv_in_channels = (
                    last_layer_dim if i == 0 else last_layer_dim*(2**i))
                branch_convs.append(
                    ConvModule(
                        conv_in_channels,
                        conv_in_channels*2,
                        ...
                        conv_cfg=self.conv_cfg,
                        norm_cfg=self.norm_cfg))
            last_layer_dim = last_layer_dim*(2**num_branch_convs)
        # add branch specific fc layers
        branch_fcs = nn.ModuleList()
        if num_branch_fcs > 0:
            # for shared branch, only consider self.with_avg_pool
