...
class CascadeMultiRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        super(CascadeMultiRoIHead, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if i < self.num_stages - 1:
                ...
                if isinstance(self.bbox_head[i], MultiInstanceBBoxHead):
                ...
                with torch.no_grad():
                    ...
                    if self.bbox_head[i].custom_activation:
                        cls_score = self.bbox_head[i].loss_cls.get_activation(
                            cls_score)
                    roi_labels = torch.where(                               #最后一个class是背景，如果bbox类别是背景则选择除此之外的最高概率的类别
                        roi_labels == self.bbox_head[i].num_classes, 
                        cls_score[:, :-1].argmax(1), roi_labels)
                    proposal_list = self.bbox_head[i].refine_bboxes_(
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        for i in range(self.num_stages):
            ...
            num_proposals_per_img = tuple(
                len(proposals) if not (isinstance(self.bbox_head[0], MultiInstanceBBoxHead) and i == 1) else 2*len(proposals)
            ...
            if i < self.num_stages - 1:
                ...
                rois = torch.cat([
                    self.bbox_head[i].regress_by_class_(rois[j], bbox_label[j],
                                                       bbox_pred[j],
                                                       img_metas[j])
                    for j in range(num_imgs)
                ])
    ...
    def get_bboxes(self, 
        ...
        # multihead.get_bboxes
        multihead = self.bbox_head[0]

        cls_score = cls_score.reshape(-1, multihead.num_classes + 1)
        bbox_pred = bbox_pred.reshape(-1, 5)
        # roi = roi.repeat_interleave(multihead.num_instance, dim=0)
        ...
        if rcnn_test_cfg is None:
            ...
            roi_idx = np.tile(
                np.arange(bboxes.shape[0] / multihead.num_instance)[:, None],
                (1, multihead.num_instance)).reshape(-1, 1)[:, 0]
            roi_idx = torch.from_numpy(roi_idx).to(bboxes.device).reshape(
                -1, 1).to(bboxes.dtype)
            bboxes = torch.cat([bboxes, roi_idx], dim=1)
            det_bbox, det_score = multihead.set_nms(
