...
class RotatedMidpointOffsetCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        Args:
            bboxes (torch.Tensor): oriented boxes. Shape (B, N, 5) or (N, 5)
...
def bbox2delta(proposals,
    ...
    pw = proposals[..., 2]
    ph = proposals[..., 3]
    pa = proposals[..., 4].unsqueeze(1)

    aobb, poly = obb2aobb(gt, pa), obb2poly(gt, version)
    gx = aobb[..., 0]
    gy = aobb[..., 1]
    gw = aobb[..., 2]
    gh = aobb[..., 3]
    ...
    x_rela = x_coor - gx.unsqueeze(1)
    y_rela = y_coor - gy.unsqueeze(1)
    x_coor = x_rela * torch.cos(pa) - y_rela * torch.sin(pa) + gx.unsqueeze(1)
    y_coor = x_rela * torch.sin(pa) + y_rela * torch.cos(pa) + gy.unsqueeze(1)
...
def delta2bbox(rois,
    ...
    obboxes[..., 4] = norm_angle(obboxes[..., 4] + pa, version)
    return obboxes
